...
function startfavpage(folderid, detectagain) {
    chrome.bookmarks.getSubTree(folderid, function(foldercfg) {
        ...
        function selectcapst(thishere) {
            chrome.bookmarks.get($(thishere).attr("folderid"), function(selectfavfic) {
        ...
        if (detectagain == "again") {} else { $("#search").keyup(function() { searchimages($(this).val()); }); }
                            ...
                            if (this.url.startsWith("https://fanfiction.com.br/")) {
                                ...
                                $(fanfic).find("#chap_select:first-child option").each(function() {
                                    ...
                                    $("#capselect").append($("<option>", {
                                        url: geturlcap + $(this).attr("value") + geturlcaprs,
                                        value: $(this).attr("value"),
                                        id: "capdivpx" + $(this).attr("value")
                                    }).text($(this).text()));
                                    numbercaplist = Number($(this).attr("value"))

                                })

                                for (i = 0; i < numbercaplist; i++) {
                                    var numxsd3 = i + 1;
...
function searchfolder(detectagain) {
    chrome.bookmarks.search({ "title": chrome.i18n.getMessage("app_fb_folder") }, function(favdata) {
        if (favdata[0] == null) {
            chrome.bookmarks.create({ "parentId": "1", "title": chrome.i18n.getMessage("app_fb_folder") }, function(newfoldersystem) {
...
$("#excodefolder").click(function() {
    if ($('#folderlist .active').attr('folderidreg') != undefined) {
        chrome.bookmarks.getSubTree($('#folderlist .active').attr('folderidreg'), function(foldercfg) {
            ...
            var json_save_file_st = { 'service': 'super_favbook_plus_fb', 'title': foldercfg[0].title, 'items': items_folder_create_file };
...
$("#impcodefolder").click(function() {
    if ($("#codepekxed").val() != '') {
        chrome.bookmarks.search({ "title": chrome.i18n.getMessage("app_fb_folder") }, function(favdata) {
            ...
            if (json_save_file_jt.service == "super_favbook_plus_fb") {
...
function loadsystemcssclick() {
    chrome.storage.local.get({ loadsync: 'local', ffb_css: '' }, function(settings) {
        if (settings.loadsync == 'sync') {
            chrome.storage.sync.get({ ffb_css: '' }, function(settings2) {
