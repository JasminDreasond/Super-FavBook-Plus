...
function antirepeat(data1, data2, type) {
    ...
    $("" + data1 + ":contains(" + data2 + ")").each(function() {
        if (duplicateChk.hasOwnProperty(this.id)) {
            if (type == 'remove') { $(this).remove(); } else if (type == 'hide') { $(this).addClass('hide'); }
        } else {
...
function loadcustomcssst(data) {
    $("#ctcss_appdevianplus").val(data.dp);
    $("#ctcss_appsoundbookplus").val(data.sb);
    $("#ctcss_appfanficbookinplus").val(data.ffb);
    $("#ctcss_appdevianplus").trigger('change');
}
...
function loadsystemcssclick() {
    chrome.storage.local.get({ loadsync: 'local', dp_css: '', sb_css: '', ffb_css: '' }, function(settings) {
        if (settings.loadsync == 'sync') {
            chrome.storage.sync.get({ dp_css: '', sb_css: '', ffb_css: '' }, function(settings2) {
...
$("#savecustomcss").click(function() {
    chrome.storage.local.get({ loadsync: 'local', dp_css: '', sb_css: '', ffb_css: '' }, function(settings) {
        $("#csserror").empty().addClass('hide');
        $(".csserror").removeClass("csserror");
...
chrome.storage.local.get({ api_youtube: true, api_soundcloud: true, api_dailymotion: true }, function(data) {
    $("[id='api_change']").each(function() {
        if ($(this).attr("api") == "youtube") { $(this).prop("checked", data.api_youtube); } else if ($(this).attr("api") == "soundcloud") { $(this).prop("checked", data.api_soundcloud); } else if ($(this).attr("api") == "dailymotion") { $(this).prop("checked", data.api_dailymotion); }
    });
});
