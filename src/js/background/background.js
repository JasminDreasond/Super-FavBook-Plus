...
function startmenu() {
    ...
    function installcodecss(data, failcode1, failcode2) {
        ...
        if (data != "fail") {
            ...
            if (checkeditemtoinstall == true) {
                chrome.storage.local.set({ download_code_css: data }, function() {
    ...
    function installthemest(data, type) {
        ...
        if (type == true) {
            $.ajax({ cache: false, dataType: "text", url: data })
        ...
        if (data3 == true) {
            chrome.contextMenus.create({
                ...
                onclick: function(click_data) {
                    if (data.type == "default") { chrome.windows.create({ url: data.url, type: "popup", state: "maximized" }); } else if (data.type == "get_url") {
                        if (click_data.linkUrl != null) { installthemest(click_data.linkUrl, true); } else if (click_data.pageUrl != null) { installthemest(click_data.pageUrl, true); } else if (click_data.frameUrl != null) { installthemest(click_data.frameUrl, true); }
    ...
    // URL
    function loadmorerightclick() {
        chrome.storage.local.get({ loadrightclick: true }, function(settings) {
